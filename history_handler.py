        ...
        for model_name, path in history_paths.items():
            try:
                ...
                history_data[model_name] = {
                    ...
                    'val_iou_metric': data['val_iou_metric']
                }
            except KeyError as e:
                print(f"Error: Missing expected key {e} in history file {path}")
                history_data[model_name] = {}
            except Exception as e:
                print(f"Failed to load history data for model '{model_name}' from path '{path}': {e}")
                history_data[model_name] = {}
        return history_data
